module tb;
    ...
    int last_instr;
    int fail_flag;
    int timeout_counter; // Added to catch infinite stall bugs
    ...
    initial begin
        ...
        ipc = 0;
        next_ipc = 1; // Initializes 1-delay slot buffer
        fail_flag = 0;
        timeout_counter = 0;

        // We calculate 23 / 5. Expectation: HI (Remainder) = 3, LO (Quotient) = 4
        instructions[0]  = 32'b00100000000010000000000000010111;  // addi $t0, $zero, 23
        instructions[1]  = 32'b00100000000010010000000000000101;  // addi $t1, $zero, 5
        instructions[2]  = 32'b00000000000000000000000000000000;  // nop 
        instructions[3]  = 32'b00000000000000000000000000000000;  // nop
        instructions[4]  = 32'b00000000000000000000000000000000;  // nop 
        instructions[5]  = 32'b00000001000010010000000000011010;  // div $t0, $t1 
        instructions[6]  = 32'b00000000000000000000000000000000;  // nop
        instructions[7]  = 32'b00000000000000000000000000000000;  // nop
        instructions[8]  = 32'b00000000000000000101000000010000;  // mfhi $t2 (Expects 3)
        instructions[9]  = 32'b00000000000000000101100000010010;  // mflo $t3 (Expects 4)
        ...
        instructions[13] = 32'b00001000000000000000000000001110;  // j 14 (Target is now 14: 001110)
        instructions[14] = 32'b00000000000000000000000000000000;  // nop (Delay slot)
        ...
        while (ipc < last_instr && !fail_flag) begin
            #2;
            if (instructions[ipc][31:26] == 6'b000000 && instructions[ipc][5:0] == 6'b011010) begin
                $display("ipc : %d", ipc);
                repeat (33) begin
                    #2;
                end
            ...
            if (InstDone === 1'b1) begin
                ...
                for (j = 1; j < 32; j++) begin
                    if (R[j] !== ireg[j]) begin
                        fail_flag = 1; 
                        $display("failed at register %d", j);
                    end
                end
                ...
                $display("Instruction didn't finish in correct clock cycles!");
                fail_flag = 1;
            end
        end
        if (!fail_flag) begin
